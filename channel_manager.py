...
class ChannelManagerWindow(QMainWindow):
    ...
    def _load_data(self):
        ...
        # Сортируем по xml_name
        self.channels.sort(key=lambda x: x['xml_name'].lower())
    ...
    def _add_missing_from_xml(self):
        ...
        for xml_name in sorted(self.channels_from_xml.keys()):
            if xml_name not in existing:
                # Пытаемся найти display-name в данных из XML
    ...
    def _filter_channels(self, text: str):
        ...
        for row in range(self.table.rowCount()):
            item = self.table.item(row, 0)  # Ищем по названию из week
            if item:
                channel = item.text().lower()
                # Также ищем в отображаемом имени
    ...
    def _add_from_xml(self):
        """Добавляет все каналы из week.xml, которых нет в списке"""
        if not self.channels_from_xml:
    ...
    def _save_channels(self):
        """Сохраняет каналы в channel_config.txt"""
    ...
    def _update_config(self):
        """Обновляет конфиг для использования в главной программе"""
        if not self.config:
            return
        
        # Обновляем словари в конфиге
    ...
    def get_channel_numbers(self) -> Dict[str, str]:
        """Возвращает словарь канал->номер (только активные)"""
        result = {}
        for channel in self.channels:
    ...
    def get_channel_names(self) -> Dict[str, str]:
        """Возвращает словарь канал->название для вывода (только активные)"""
        result = {}
        for channel in self.channels:
